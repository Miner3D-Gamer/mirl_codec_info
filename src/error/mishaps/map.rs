...
impl std::fmt::Display for ParserMapMishap {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let message = match self {
            Self::DataAfterKeyElement => {
                "Value after Key element - Did you forget a colon?"
            }
            Self::DataAfterValueElement => {
                "Value after Key element - Did you forget a comma?"
            }
            Self::InvalidKeyType => {
                "Expected Key element that is not supported"
            }
